        ...
        if (parser != null) {
            ...
            switch (requestType) {
                ...
                case GET_LIST_OF_SPOTIFY_SONGS:
                    ((RoomActivity) callingActivity).setListOfSongsFromSearch((ArrayList<Song>) parsedResult);
                    break;

                case GET_LIST_OF_ROOMS:
                    ((SelectRoomActivity) callingActivity).setListOfRooms((ArrayList<Room>) parsedResult);
                    break;
                case GET_ROOM_INFOS:
